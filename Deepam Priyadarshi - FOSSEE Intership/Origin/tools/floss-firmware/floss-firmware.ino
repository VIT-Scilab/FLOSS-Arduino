...
/* create and initialize LCD in 4-bit I/O mode 
  rs pin      -> 12
  rw pin      -> 10
  enable pin  -> 11
  d4 pin      -> 5
...
void loop() {
  ...
  int charCount = 0;         /* characters displayed on LCD*/
...
//            Serial.println(dgv);    
  ...
  //case L -> LCD
    ...
    val = Serial.read();
    if (val == 115){//'s'-> size of lcd, columns and rows
      int cols = Serial.read();
      int rows = Serial.read();
      lcd.begin(cols, rows);
    }
    if (val == 116){//'t'-> received text message
      lcd.clear();
      charCount = 0;
      while(true){
        while(Serial.available()==0){};
      ...
      if(charCount > 16){// scrooling of text msg if characters > 16
        delay(800);
        for(int i=16; i<charCount; i++){
